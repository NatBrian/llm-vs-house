// Authentic Sic Bo table — cleaned-up faithful layout matching the same bet-family
// grouping/adjacency as the real GRA "SIC BO (MBS) Game Rules Version 7" Appendix A/B
// felt (Small/Big at the outer edges, doubles+specific-triples+Any Triple in the middle
// band, a totals row, the two-dice combination grid, then the single-number row) but
// reflowed for a browser rather than pixel-matched to the dense physical layout. Four
// bands, top → bottom:
//   1. SMALL | doubles + specific-triples + ANY TRIPLE | BIG
//   2. Totals 4..17 (each with its exact GRA "1 wins N" odds)
//   3. Fifteen two-dice combinations (dominoes), 1 wins 6
//   4. Single numbers ONE..SIX (1:1 / 2:1 / 12:1 by match count)
// All payouts verified directly against GRA rule 4.1 (docs/PAYOUTS.md §3) — several
// differ from the generic "Wizard of Odds standard" table most secondary sources quote.
//
// It is a *replay* board: the bot's placedBets drop chips on the matching cells, the
// three dice tumble inside a glass shaker dome, the dome lifts, winning cells light up
// in a stagger keyed to payout size (biggest hits light last, with extra flash), and a
// result banner pops. Below the felt, a hot/cold trend panel (recent-rolls strip +
// Small/Big/Triple and Odd/Even/Triple frequency bars + a 1-6 hot/cold tracker) mirrors
// the RouletteBoard/BaccaratBoard history-panel pattern. Win predicates here mirror
...
// ---------------------------------------------------------------- shaker dome
// The signature Sic Bo look: three dice vibrate under a glass dome on a lit pedestal,
// the dome lifts, the dice settle with a bounce, then a brief glow-flash beat before the
// result banner pops — gives the reveal a moment of held tension instead of an instant cut.
...
/**
 * Winning-cell reveal delay, keyed to the bet's payout multiple — cheap even-money
 * hits (Small/Big/Combo, 1-6:1) light almost instantly, a specific Triple (180:1)
 * lights last. Gives the celebration a natural "biggest win lands last" build instead
 * of every cell flashing at once. `big` (payout >= 30:1) gets an extra flash burst.
 */
...
export function SicBoBoard({ dice, placedBets, history = [], roundKey }: {
  dice: number[]; placedBets: any[]; history?: number[][]; roundKey: number;
}) {
  ...
  // GRA "SIC BO (MBS) Game Rules Version 7" rule 4.1.2 — exact gazetted odds
  const TOTAL_ODDS: Record<number, number> = {
    4: 62, 5: 31, 6: 18, 7: 12, 8: 8, 9: 7, 10: 6, 11: 6, 12: 7, 13: 8, 14: 12, 15: 18, 16: 31, 17: 62,
  };
  const combos: [number, number][] = [];
  for (let x = 1; x <= 6; x++) for (let y = x + 1; y <= 6; y++) combos.push([x, y]);

  const DoubleCell = (f: number) => (
    <Cell key={`d${f}`} state={st(`double-${f}`)} win={winDouble(f)} reveal={reveal} odds={11} title={`Double ${f} — 1 wins 11`} className="flex-1 py-1 gap-1">
  ...
  const TripleCell = (f: number) => (
    <Cell key={`t${f}`} state={st(`triple-${f}`)} win={winTriple(f)} reveal={reveal} odds={180} title={`Triple ${f}${f}${f} — 1 wins 180`} className="flex-1 py-1 gap-0.5">
    ...
    <div className="w-full flex flex-col items-center gap-3">
      <div className="w-full overflow-x-auto">
        <div className="mx-auto flex flex-col gap-3" style={{ minWidth: 720 }}>
          ...
          <div className="relative rounded-lg overflow-hidden border-2 border-gold-500/60 select-none shadow-[0_10px_40px_rgba(0,0,0,0.5)]"
            ...
            <div className="grid" style={{ gridTemplateColumns: '2.2fr 3fr 3fr 1.9fr 3fr 3fr 2.2fr', minHeight: 96 }}>
              ...
              <div className="flex flex-col border-x border-gold-600/40">
                <div className={grpHdr}>Each double 1 wins 11</div>
              ...
              {/* any triple */}
              <div className="flex flex-col border-x border-gold-600/40">
                <div className={grpHdr}>1 wins 31</div>
                <Cell state={st('anytriple')} win={triple} reveal={reveal} odds={31} className="flex-1 p-1" title="Any triple — 1 wins 31">
            ...
            <div className="grid border-t-2 border-gold-500/60" style={{ gridTemplateColumns: `repeat(14,1fr)`, minHeight: 46 }}>
              {Object.keys(TOTAL_ODDS).map((k) => {
                const n = Number(k);
                return (
                  <Cell key={n} state={st(`total-${n}`)} win={winTotal(n)} reveal={reveal} odds={TOTAL_ODDS[n]} className="py-1 border-l border-gold-600/30" title={`Total ${n} — 1 wins ${TOTAL_ODDS[n]}`}>
            ...
            <div className="grid border-t-2 border-gold-500/60" style={{ gridTemplateColumns: `2fr repeat(15,1fr)`, minHeight: 52 }}>
              <div className="flex flex-col items-center justify-center border-r border-gold-600/40 px-1 text-center">
                ...
                <span className="text-[8px] text-gold-100/90 mt-0.5">1 wins 6</span>
              </div>
              {combos.map(([x, y]) => (
                <Cell key={`${x}-${y}`} state={st(`combo-${x}-${y}`)} win={winCombo(x, y)} reveal={reveal} odds={6} className="py-1 gap-0.5 border-l border-gold-600/30" title={`${x} & ${y} — 1 wins 6`}>
