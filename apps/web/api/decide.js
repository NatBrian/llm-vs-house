...
function rouletteWheel(variant) {
  const base = [];
  for (let n = 0; n <= 36; n++) base.push(n);
  if (variant === "american") base.push("00");
  return base;
}
function spinRoulette(rng, variant) {
...
var Shoe = class _Shoe {
  constructor(cards) {
    this.cards = cards;
  }
  cards;
  idx = 0;
  static shuffled(decks, rng) {
    return new _Shoe(shuffle(buildShoe(decks), rng));
  }
  /** For tests: build directly from a fixed list (top of shoe = index 0). */
  static fromCards(cards) {
    return new _Shoe(cards.slice());
  }
  draw() {
...
// ../../packages/engine/src/games/baccarat.ts
...
function playBaccaratCoup(shoe) {
  ...
  const result = pt === bt ? "tie" : pt > bt ? "player" : "banker";
...
function newHand(cards, bet, fromSplit = false, splitAce = false) {
...
function startBlackjack(rng, rules, bet) {
  return startBlackjackFromShoe(Shoe.shuffled(rules.decks, rng), rules, bet);
}
function startBlackjackFromShoe(shoe, rules, bet) {
  const p1 = shoe.draw();
  const d1 = shoe.draw();
  const p2 = shoe.draw();
  const d2 = shoe.draw();
...
function resolveNaturals(state) {
...
function legalActions(state) {
  ...
  const canSplit = twoCards && !h.splitAce && bjCardValue(h.cards[0]) === bjCardValue(h.cards[1]) && state.hands.length < state.rules.maxSplitHands;
...
function dealerPlayAndSettle(state) {
  ...
  if (anyLive) {
    while (true) {
      ...
      if (total === 17 && soft && state.rules.dealerHitsSoft17) {
        state.dealer.push(state.shoe.draw());
        continue;
      }
      break;
...
function settle(state) {
  ...
  const handNets = state.hands.map((h) => {
    ...
    if (playerBJ) {
      ...
      return h.bet * state.rules.blackjackPayout;
    }
    if (dealerBJ) return -h.bet;
    if (dealerBust) return h.bet;
    const total2 = handValue(h.cards).total;
    if (total2 > dealerTotal) return h.bet;
    if (total2 < dealerTotal) return -h.bet;
    return 0;
  });
  const insuranceNet = state.insuranceBet > 0 ? dealerBJ ? state.insuranceBet * 2 : -state.insuranceBet : 0;
...
async function ask(ctx, req) {
  ...
  const parsed = req.schema.parse(res.value);
  const reasoning2 = typeof parsed.reasoning === "string" ? parsed.reasoning : "";
  const step = {
    kind: req.kind,
    ...
    ...res.raw !== void 0 ? { raw: res.raw } : {},
    ...res.meta !== void 0 ? { meta: res.meta } : {}
  };
...
var baccaratAdapter = {
  ...
  async playRound(ctx) {
    ...
    return {
      steps: [step],
      outcome: {
        player: labels(coup.player),
        banker: labels(coup.banker),
...
var blackjackAdapter = {
  id: "blackjack",
  label: "Blackjack",
  defaultConfig: () => DEFAULT_BLACKJACK_RULES,
  async playRound(ctx) {
    const rules = ctx.config;
    const steps = [];
    const betReq = {
      kind: "bet",
      game: "blackjack",
    ...
    while (state.phase !== "settled") {
      ...
      const actReq = {
        ...
        legalActions: legal,
        schema: BlackjackActionSchema,
        schemaName: "BlackjackAction"
      };
      const res = await ask(ctx, actReq);
      const applied = legal.includes(res.value.action) ? res.value.action : legal[0];
...
var GAME_LABELS = Object.fromEntries(
...
// ../../node_modules/.pnpm/nanoid@3.3.15/node_modules/nanoid/non-secure/index.js
...
var AnthropicMessagesLanguageModel = class {
  ...
  async doGenerate(options) {
    ...
    const reasoning2 = response.content.filter(
    ...
    return {
      text: text2,
      reasoning: reasoning2.length > 0 ? reasoning2 : void 0,
      toolCalls,
...
// ../../node_modules/.pnpm/@ai-sdk+openai-compatible@0.2.16_zod@3.25.76/node_modules/@ai-sdk/openai-compatible/dist/index.mjs
...
var OpenAICompatibleChatLanguageModel = class {
  ...
  constructor(modelId, settings, config2) {
    ...
    this.supportsStructuredOutputs = (_b = config2.supportsStructuredOutputs) != null ? _b : false;
  }
  get defaultObjectGenerationMode() {
...
function createOpenAICompatible(options) {
  ...
  const createLanguageModel = (modelId, settings = {}, config2) => createChatModel(modelId, settings, config2);
  const createChatModel = (modelId, settings = {}, config2) => new OpenAICompatibleChatLanguageModel(modelId, settings, {
  ...
  const provider = (modelId, settings, config2) => createLanguageModel(modelId, settings, config2);
  provider.languageModel = createLanguageModel;
...
var OLLAMA_DEFAULT_BASE = "http://localhost:11434/v1";
var OPENROUTER_BASE = "https://openrouter.ai/api/v1";
var KILOCODE_BASE = "https://api.kilo.ai/api/gateway";
var GATEWAY_PROVIDERS = /* @__PURE__ */ new Set(["ollama", "openrouter", "kilocode", "openai-compatible"]);
function requireBaseURL(cfg) {
...
// ../../node_modules/.pnpm/ai@4.3.19_react@19.2.7_zod@3.25.76/node_modules/ai/dist/index.mjs
...
function createLlmDecide(cfg, deps = {}) {
  const model = resolveModel(cfg);
  const gen = deps.generateObject ?? ((args) => generateObject({ ...args, model }));
  ...
  return async (req) => {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        const value = req.schema.parse(result.object);
        return {
          value,
...
async function handleDecide(payload) {
  ...
  const apiKey = payload.apiKey || (envVar ? process.env[envVar] : void 0);
